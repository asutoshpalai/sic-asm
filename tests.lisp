...
(define-test asm-firstpass
            ...
            (multiple-value-bind (symtab start len) (pass1 "testfiles/test1.asm")
              (assert-equal 1006 (gethash "inloop" symtab))
              (assert-equal 1018 (gethash "outlp" symtab))
              (assert-equal 1039 (gethash "indev" symtab))
              (assert-equal 1040 (gethash "outdev" symtab))
              (assert-equal 1041 (gethash "data" symtab))
              (assert-equal 1042 (gethash "dest" symtab))
              (assert-equal 1045 (gethash "four" symtab))
              (assert-equal 1048 (gethash "nine" symtab))
